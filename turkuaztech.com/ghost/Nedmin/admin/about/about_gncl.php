...
<?php
  ...
  $gncl = $db->prepare("SELECT * FROM wp_about WHERE id=:id");

  $gncl->execute(array(":id" => $id));

  $row = $gncl->fetch(PDO::FETCH_ASSOC);
...
<form action="" method="post">
 <div class="about-box-area">
   <div style="width: 100%;height: auto;float: left;">
   	<div class="file_text">
	   Kapak Resmi Yükle
	   <input value="<?php echo $row['img'];?>" type="file" name="img" />
	</div>
   </div>
   <div style="width: 100%;height: auto;float: left;margin-top: 15px;">
     <textarea name="editor1">
     	<?php echo $row['editor1'];?>
     </textarea>
	 <script>
...
if($_POST){
    ...
    $gncl = $db->prepare("UPDATE wp_about set 
    ...
    if($x){
    	echo '
         <script type="text/javascript">alert("Hakkında Başarıyla Güncellendi:)");</script>
    	';
    }else{
    	echo "Başarısız";
    }
