...
<div class="admin-page-left-menu">
	...
	<div style="width: 100%;height: auto;float: left;margin-top: 8px;">
 	...
 	<?php //Makale sayac
 	$sayac=0; 
    $sorgu = $db->query("SELECT * FROM wp_blog",PDO::FETCH_ASSOC);
	if($sorgu->rowCount())
    {
    ...
    <?php //Admin sayac
 	$sayac2=0; 
    $sorgu = $db->query("SELECT * FROM user_admin",PDO::FETCH_ASSOC);
	if($sorgu->rowCount())
    {
        foreach($sorgu as $icerik)
        {
    ...
    <?php //Mesaj sayac
 	$sayac3=0; 
    $sorgu = $db->query("SELECT * FROM wp_message",PDO::FETCH_ASSOC);
	if($sorgu->rowCount())
    {
        foreach($sorgu as $icerik)
        {
    ...
    <?php //Proje sayac
 	$sayac4=0; 
    $sorgu = $db->query("SELECT * FROM wp_proje",PDO::FETCH_ASSOC);
	if($sorgu->rowCount())
    {
        foreach($sorgu as $icerik)
        {
          ...
          //yorum sayac
 	$sayac5=0; 
    $sorgu = $db->query("SELECT * FROM bg_yorum",PDO::FETCH_ASSOC);
	if($sorgu->rowCount())
    {
        foreach($sorgu as $icerik)
        {
...
$bloglar = $db -> query("SELECT * FROM wp_blog");
 
if($bloglar -> rowcount()){
    foreach($bloglar as $bloglar_db_gelen){
 
        $blog_id = $bloglar_db_gelen['bg_hit'];
...
<div class="admin_page_status-box">
	<i class="far fa-newspaper"></i>
 	<h2><?php echo $sayac; ?> Makale</h2>
</div>
<div class="admin_page_status-box">
	<i class="fas fa-project-diagram"></i>
	<h2><?php echo $sayac4; ?> Proje</h2>
</div>
<div class="admin_page_status-box">
	<i class="far fa-envelope"></i>
	<h2><?php echo $sayac3; ?> Yeni Mesaj</h2>
</div>
<div class="admin_page_status-box">
	<i class="fas fa-comment"></i>
	<h2><?php echo $sayac5; ?> Yorum</h2>
</div>
<div class="admin_page_status-box">
	<i class="far fa-eye"></i>
	<h2><?php echo $blog_id; ?> Görüntülenme</h2>
</div>
<div class="admin_page_status-box">
	<i class="fas fa-user-shield"></i>
	<h2><?php echo $sayac2; ?> Admin</h2>
</div>
