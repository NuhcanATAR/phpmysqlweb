...
$mkl_list = $db->query('SELECT * FROM wp_blog');

$mkl_list->execute(array());

$row = $mkl_list->fetchAll(PDO::FETCH_ASSOC);

foreach($row as $rows){

?>

<div class="admin_page_blog_box">
	<div style="width: 100%;height: auto;float: left;">
		<h1><?php echo $rows['bg_baslik'];?></h1>
		<p>Yazar:<?php echo $rows['bg_yzr'];?></p><p>Tarih: <?php echo $rows['bg_gun'];?> <?php echo $rows['bg_ay'];?> <?php echo $rows['bg_yil'];?></p><p>İçerik Konusu:<?php echo $rows['bg_ktlg'];?></p>
	</div>
	<div style="width: 100%;height: auto;float: left;">
		<img src="img/<?php echo $rows['bg_kapak'];?>">
	</div>
	<div style="width: 100%;height: auto;float: left;">
		<a href="mkl_gncl.php?id=<?php echo $rows['id']; ?>"><button>Düzenle</button></a>
		<a href="mkl_sil.php?i=<?php echo $rows['id'];?>"><button style=" background-color: rgb(255, 31, 31);">Kaldır</button></a>
	</div>
</div>
...
<div class="admin-page-right-menu">
	<a href="mkl-ekl.php">
	<div class="blg-add-btns">
		<i class="fas fa-plus"></i>
		<p>Yeni Makale Oluştur</p>
	</div>
    </a>
