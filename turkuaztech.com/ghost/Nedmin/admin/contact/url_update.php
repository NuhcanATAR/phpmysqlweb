...
$data_list = $db->prepare("SELECT * FROM wp_contact_url where id=:id");

$data_list->execute(array(":id" => $id));

$row = $data_list->fetch(PDO::FETCH_ASSOC);
...
<ol style="list-style-type:none;">
  <li><input type="text" required="" value="<?php echo $row["url_instagram"];  ?>" placeholder="İnstagram Url" name="url_instagram"></li>

  <li><input type="text" required="" value="<?php echo $row["url_facebook"];  ?>" placeholder="Facebook Url" name="url_facebook"></li>

  <li><input type="text" required="" value="<?php echo $row["url_twitter"];  ?>" placeholder="Twitter Url" name="url_twitter"></li>

  <li><input type="text" required="" value="<?php echo $row["url_mail"];  ?>" placeholder="Mail Adres" name="url_mail"></li>
...
if($_POST){
  ...
  $url_add = $db->prepare("UPDATE wp_contact_url set
  ...
  if($url_control){
    echo '
    <script type="text/javascript">alert("URL Adresleri Güncellendi");</script>
    ';
    header("refresh:1;url=connect-list.php")
  }else{
    echo "Başarısız";
  }
