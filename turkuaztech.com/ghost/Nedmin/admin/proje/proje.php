...
$pr_list = $db->query('SELECT * FROM wp_proje');

$pr_list->execute(array());

$row = $pr_list->fetchAll(PDO::FETCH_ASSOC);

foreach($row as $rows){

?>
<div style="margin-top:5px;" class="proje-list-box">
  <div style="width:100%;height:auto;float:left;">
    <h2><?php echo $rows['pr_baslik'];?></h2>
  </div>
  <div style="width:100%;height:auto;float:left;">
     <div style="background-image:url('img/<?php echo $rows['pr_kapak'];?>');" class="proje_kapak_box_img"></div>
  </div>
  <div style="width:100%;height:auto;float:left;">
    <a href="proje_dznl.php?id=<?php echo $rows['id'];?>">
      <button>Düzenle</button>
    </a>
    <a href="proje_sil.php?i=<?php echo $rows['id'];?>">
      <button style="float:right;margin-right:8%; background-color: rgb(247, 28, 28);">Sil</button>
    </a>
  </div>
</div>
<?php } ?>
