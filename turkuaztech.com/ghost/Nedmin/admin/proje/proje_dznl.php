...
$pr_sorgu = $db->prepare('SELECT * FROM wp_proje WHERE id =:id');

$pr_sorgu->execute(array(':id' => $id));

$rows = $pr_sorgu->fetch(PDO::FETCH_ASSOC);

?>

<form action="" method="post">
<div class="proje_add_input_box">
  <div style="width:100%;height:auto;float:left;">
    <div class="file_text">
   	  Kapak Resmi Yükle
   	  <input type="file" value="<?php echo $rows['pr_kapak'];?>" name="pr_kapak" />
   	</div>
  </div>
  <div style="width:100%;height:auto;float:left;">
    <input type="text" value="<?php echo $rows['pr_baslik'];?>" required="" name="pr_baslik" placeholder="Proje İsmi"/>
  </div>
  <div style="width:100%;height:auto;float:left;">
    <input type="text" required="" value="<?php echo $rows['pr_ozet'];?>" name="pr_ozet" placeholder="Proje Özetiniz"/>
  </div>
  <div style="width:100%;height:auto;float:left;">
    <textarea name="editor1"><?php echo $rows['editor1'];?></textarea>

  </div>
  <div style="width:100%;height:auto;float:left;">
    <button>Projeyi Ekle</button>
  </div>
</div>
</form>
...
if($_POST){
  ...
  $pr_kyt = $db->prepare("UPDATE wp_proje set
  ...
  if($pr_control){
    echo '	<script type="text/javascript">alert("Projeniz Başarıyla Güncellendi:)");</script>';
...
<div class="admin-page-right-menu">
    ...
    <a href="proje_ekl.php">
     <div class="proje_add_btns">
       <i class="fas fa-plus"></i>
       <p>Yeni Projeni Ekle</p>
     </div>
    </a>
</div>
